...
* **外部整合工具 (external integration tools)** 專案風險控制策略，用於專案複雜度高時，例如 JAD、終端使用者參與。（外部利害關係人）
...
* **內部整合工具 (internal integration tools)** 專案風險控制策略，用於專案複雜度高時，例如設立資訊系統專家與使用者間的聯繫團隊。（內部團隊）
