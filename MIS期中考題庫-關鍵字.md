...
* **外部整合工具 (external integration tools)** 專案風險控制策略，用於專案複雜度高時，例如 JAD、終端使用者參與。
* **正式控制工具 (formal control tools)** 專案風險控制策略，用於大型或複雜專案，例如檢查點、書面文件、導入後稽核。
* **正式規劃工具 (formal planning tools)** 專案風險控制策略，用於規劃與控制專案，例如 PERT 圖、甘特圖。
* **甘特圖 (Gantt chart)** 正式規劃工具，以長條圖方式，視覺化呈現專案活動的時程與資源分配。
* **專人 (implementation)** 新資訊系統引進組織時，所引發的組織改變之全部過程。
...
* **內部整合工具 (internal integration tools)** 專案風險控制策略，用於專案複雜度高時，例如設立資訊系統專家與使用者間的聯繫團隊。
...
* **社會技術設計 (sociotechnical design)** 結合資訊系統技術解決方案與組織、管理設計的系統設計方法論。
